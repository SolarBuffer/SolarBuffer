...
_NGINX_CONF_SRC  = os.path.join(UPDATE_DIR, "solarbuffer", "nginx", "solarbuffer.conf")
_NGINX_CONF_DEST = "/etc/nginx/sites-available/solarbuffer"
_NGINX_LINK      = "/etc/nginx/sites-enabled/solarbuffer"
_AVAHI_SRC       = os.path.join(UPDATE_DIR, "solarbuffer", "avahi", "solarbuffer-cname.service")
_AVAHI_DEST      = "/etc/systemd/system/solarbuffer-cname.service"
...
def _ensure_nginx_setup():
    """Installeer en configureer nginx + avahi subdomains als dat nog niet gedaan is."""
    if os.name == "nt":
        return
    try:
        nginx_installed = _run(["which", "nginx"]).returncode == 0
        if not nginx_installed:
            _run(["sudo", "apt-get", "install", "-y", "nginx"], timeout=120)

        # Nginx config bijwerken vanuit repo
        if os.path.exists(_NGINX_CONF_SRC):
            _run(["sudo", "cp", _NGINX_CONF_SRC, _NGINX_CONF_DEST])
            _run(["sudo", "rm", "-f", "/etc/nginx/sites-enabled/default"])
            _run(["sudo", "ln", "-sf", _NGINX_CONF_DEST, _NGINX_LINK])
            if _run(["sudo", "nginx", "-t"]).returncode == 0:
                _run(["sudo", "systemctl", "enable", "--now", "nginx"])
                _run(["sudo", "systemctl", "reload", "nginx"])

        # Avahi CNAME service installeren/bijwerken
        if os.path.exists(_AVAHI_SRC):
            _run(["sudo", "cp", _AVAHI_SRC, _AVAHI_DEST])
            _run(["sudo", "systemctl", "daemon-reload"])
            _run(["sudo", "systemctl", "enable", "--now", "solarbuffer-cname"])
            _run(["sudo", "systemctl", "restart", "solarbuffer-cname"])

        write_audit_log("nginx_setup_ok", {})
    except Exception as exc:
        write_audit_log("nginx_setup_error", {"error": str(exc)})
...
if __name__ == "__main__":
    threading.Thread(target=_ensure_nginx_setup, daemon=True).start()
